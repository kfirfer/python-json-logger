...
class RequestUtil(object):
    ...
    def __new__(cls, *args, **kw):
        if not hasattr(cls, '_instance'):
            ...
            validate_subclass(request_adapter_class, custom.RequestAdapter)
            validate_subclass(response_adapter_class, custom.ResponseAdapter)
            ...
            cls._instance.create_correlation_id_if_not_exists = custom.CREATE_CORRELATION_ID_IF_NOT_EXISTS
    ...
    def get_correlation_id(self, request=None):
        ...
        It tries to search from logging.CORRELATION_ID_HEADERS list, one by one.\n
        ...
        if request is None:
            ...
            if not self.request_adapter.is_in_request_context(request):
                # _logger.debug("Not in request context, return %s", logging.EMPTY_VALUE,
                #               extra={'correlation_id': '-'})
                return custom.EMPTY_VALUE
        ...
        # exists = logging.CREATE_CORRELATION_ID_IF_NOT_EXISTS
        if correlation_id is None and self.create_correlation_id_if_not_exists:
            correlation_id = str(custom.CORRELATION_ID_GENERATOR())
            self.request_adapter.set_correlation_id(request, correlation_id)

        return correlation_id if correlation_id else custom.EMPTY_VALUE
    ...
    def _get_correlation_id_in_request_header(request_adapter, request):
        for header in custom.CORRELATION_ID_HEADERS:
